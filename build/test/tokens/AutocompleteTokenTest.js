...
describe('AutocompleteToken', function () {
    var token = null;
    beforeEach(function () {
        token = new AutocompleteToken_1.default({
            minQueryLength: 2,
    ...
    it('should respect minlength', function (done) {
        token.checkAndRemove("t").then(function (res) {
            chai_1.expect(res.autocomplete).to.have.length(0);
            done();
        });
    });
    it('should get autocomplete values', function (done) {
        token.checkAndRemove("The hobbit").then(function (res) {
            chai_1.expect(res.autocomplete).to.have.length.gt(0);
            done();
        });
    });
    it('should accept complete values as valid', function (done) {
        token.checkAndRemove("The Hobbit: An Unexpected Journey").then(function (res) {
            chai_1.expect(res.isValid).to.be.true;
            done();
        });
    });
});
//# sourceMappingURL=AutocompleteTokenTest.js.map