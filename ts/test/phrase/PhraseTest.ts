...
describe('Phrase', () => {
  ...
  it('should catch named token', () => {
    let phrase = new Phrase("{test}", { test : new StringToken ({ value : '...'})});
    expect(phrase.element).to.be.an.instanceof(SinglePhraseElement);
  ...
  it('should work with string and token', () => {
      let phrase = new Phrase("test {test}", { test : new StringToken ({ value : '...'})});
  ...
  it('should work with mixed elements', () => {
      let phrase = new Phrase("(test1) test2 {test} test3 (test4)", { test : new StringToken ({ value : '...'})});
  ...
  it('should work with everything', () => {
      let phrase = new Phrase("(i want to find|i'm looking for) {test:capture1} movies (from the year {test:capture2}|with {test:capture3}|ordered by (asc|desc)?)?{0,2}", { test : new StringToken ({ value : '...'})});
